...
function CompetitorCalendar({ competitor, dates, prices }: { competitor: RegisteredCompetitor; dates: string[]; prices: CompetitorPrice[] }) {
  ...
  return <details className="group rounded-xl border bg-white shadow-sm">
    ...
    <p className="border-t bg-amber-50 p-3 text-xs text-amber-900">OTA部屋なし日率は、取得済み日数のうち、このOTAが「満室・空室なし」を返した割合です。施設全体の実稼働率や残室数ではなく、掲載先ごとの需要シグナルとして利用します。</p>
  </details>;
}
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      void collectionRequest.then(async () => {
        ...
        else setWarning(collectionError?.response?.data?.detail || '最新データの取得に失敗しました。保存済みデータを表示しています。');
      });
