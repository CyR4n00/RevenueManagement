...
export function SettingsPanel({ apiBase, onClose }: { apiBase: string; onClose: () => void }) {
  ...
  return <section className="rounded-xl border bg-white p-6 shadow-sm"><div className="flex items-start justify-between gap-4"><div><h2 className="text-lg font-bold">運用設定</h2><p className="mt-1 text-sm text-slate-500">秘密情報は画面に保存せず、サーバー環境変数で管理します。</p></div><button onClick={onClose} aria-label="設定を閉じる" className="text-xl">×</button></div>{message && <p role="alert" className="mt-4 rounded bg-red-50 p-3 text-sm text-red-800">{message}</p>}
    ...
    <div className="mt-5 rounded border border-amber-200 bg-amber-50 p-3"><p className="text-sm font-semibold text-amber-900">OTA収集の許諾状態</p><p className="mt-1 text-xs text-amber-800">許諾済みのOTAだけが本番でApifyを実行します。顧客がURLを登録しても、許諾待ちのOTAは収集しません。</p><div className="mt-3 grid gap-2 md:grid-cols-2">{integration?.ota_sources?.map(source => <div key={source.key} className="flex items-center justify-between rounded bg-white px-3 py-2 text-sm"><span>{source.name}</span><span className={source.status === 'approved' ? 'font-semibold text-emerald-700' : source.status === 'pending' ? 'font-semibold text-amber-700' : 'font-semibold text-slate-500'}>{source.status === 'approved' ? '許諾済み' : source.status === 'pending' ? '許諾待ち' : '停止中'}</span></div>)}</div></div>
