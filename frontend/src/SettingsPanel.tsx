    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden mt-6 mb-8">
      ...
      <div className="p-6 space-y-8">
            ...
            {competitors.map((comp, index) => (
              <div key={comp.id} className="border rounded-lg p-4 bg-gray-50 flex flex-col md:flex-row items-start md:items-center space-y-3 md:space-y-0 md:space-x-4">
                 ...
                 <div className="flex-1 w-full">
                   ...
                   <input id={`comp_name_${comp.id}`} type="text" className="w-full p-2 border rounded text-sm focus:ring-2 focus:ring-blue-400 outline-none focus-visible:ring-2" defaultValue={comp.name} />
                 </div>
                 <div className="flex-2 w-full md:w-1/2">
                   ...
                   <input id={`comp_url_${comp.id}`} type="text" className="w-full p-2 border rounded text-sm focus:ring-2 focus:ring-blue-400 outline-none focus-visible:ring-2" defaultValue={comp.url} />
                 </div>
              </div>
            ))}
          </div>
          ...
          <div className="flex space-x-4">
            <div className="flex-1">
              ...
              <div className="relative">
                ...
                <input id="min_price" type="number" defaultValue="5000" className="w-full pl-8 p-2 border rounded text-sm focus:ring-2 focus:ring-blue-400 outline-none focus-visible:ring-2" />
              </div>
            </div>
            <div className="flex-1">
              ...
              <div className="relative">
                <span className="absolute left-3 top-2 text-gray-500">¥</span>
                <input id="max_price" type="number" defaultValue="30000" className="w-full pl-8 p-2 border rounded text-sm focus:ring-2 focus:ring-blue-400 outline-none focus-visible:ring-2" />
              </div>
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4">
            <div className="border p-4 rounded-lg bg-green-50 border-green-200">
               ...
               <button className="w-full bg-[#06C755] text-white font-bold py-2 rounded shadow hover:bg-green-600 transition-colors text-sm mt-2 focus-visible:ring-2">
                 LINEアカウントと連携する
               </button>
            </div>
          ...
          <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
               <div>
                 <label htmlFor="notify_threshold" className="block text-xs font-bold text-gray-500 mb-1">通知を送る「価格変動」のしきい値</label>
                 <select id="notify_threshold" defaultValue="3000" className="w-full p-2 border rounded text-sm focus:ring-2 focus:ring-blue-400 outline-none bg-white focus-visible:ring-2">
               ...
               <div>
                 <label htmlFor="notify_timing" className="block text-xs font-bold text-gray-500 mb-1">通知のタイミング</label>
                 <select id="notify_timing" defaultValue="morning" className="w-full p-2 border rounded text-sm focus:ring-2 focus:ring-blue-400 outline-none bg-white focus-visible:ring-2">
        ...
        <div className="flex justify-end pt-4 border-t">
          <button onClick={onClose} className="bg-blue-600 text-white px-6 py-2 rounded-lg font-bold shadow hover:bg-blue-700 transition-colors focus-visible:ring-2">
            設定を保存して戻る
          </button>
        </div>
      </div>
