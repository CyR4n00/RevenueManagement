...
export function SettingsPanel({ onClose, id = "settings-panel" }: { onClose: () => void, id?: string }) {
  ...
  return (
    <div id={id} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden mt-6 mb-8">
      <div className="bg-gray-50 border-b p-4 flex justify-between items-center">
        ...
        <button aria-label="閉じる" onClick={onClose} className="text-gray-500 hover:text-gray-800 font-bold text-xl focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-400 rounded">&times;</button>
      </div>
      ...
      <div className="p-6 space-y-8">
            ...
            {competitors.map((comp, index) => (
              <div key={comp.id} className="border rounded-lg p-4 bg-gray-50 flex flex-col md:flex-row items-start md:items-center space-y-3 md:space-y-0 md:space-x-4">
                 ...
                 <div className="flex-1 w-full">
                   ...
                   <input id={`comp-name-${comp.id}`} type="text" className="w-full p-2 border rounded text-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-400" defaultValue={comp.name} />
                 </div>
                 <div className="flex-2 w-full md:w-1/2">
                   ...
                   <input id={`comp-url-${comp.id}`} type="text" className="w-full p-2 border rounded text-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-400" defaultValue={comp.url} />
                 </div>
              </div>
            ))}
          </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4">
            <div className="border p-4 rounded-lg bg-green-50 border-green-200">
               ...
               <div className="flex items-center space-x-2 text-sm text-gray-700 mb-2">
                 <input id="line-notify" type="checkbox" defaultChecked={true} className="rounded text-green-600 focus:ring-green-500 focus-visible:ring-2" />
                 <label htmlFor="line-notify">LINEで通知を受け取る</label>
               </div>
               <button className="w-full bg-[#06C755] text-white font-bold py-2 rounded shadow hover:bg-green-600 transition-colors text-sm mt-2 focus:outline-none focus-visible:ring-2 focus-visible:ring-green-800">
                 LINEアカウントと連携する
               </button>
            </div>
            ...
            <div className="border p-4 rounded-lg bg-gray-50">
               ...
               <div className="flex items-center space-x-2 text-sm text-gray-700 mb-2">
                 <input id="email-notify" type="checkbox" defaultChecked={false} className="rounded text-blue-600 focus:ring-blue-500 focus-visible:ring-2" />
                 <label htmlFor="email-notify">メールで通知を受け取る</label>
               </div>
               <label htmlFor="email-address" className="sr-only">メールアドレス</label>
               <input id="email-address" type="email" placeholder="example@hotel.com" className="w-full p-2 border rounded text-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-400 mt-1" />
            </div>
          </div>
          ...
          <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
               <div>
                 <label htmlFor="price-threshold" className="block text-xs font-bold text-gray-500 mb-1">通知を送る「価格変動」のしきい値</label>
                 <select id="price-threshold" defaultValue="3000" className="w-full p-2 border rounded text-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-400 bg-white">
               ...
               <div>
                 ...
                 <select id="notify-timing" defaultValue="morning" className="w-full p-2 border rounded text-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-400 bg-white">
        ...
        <div className="flex justify-end pt-4 border-t">
          <button onClick={onClose} className="bg-blue-600 text-white px-6 py-2 rounded-lg font-bold shadow hover:bg-blue-700 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-800">
            設定を保存して戻る
          </button>
        </div>
      </div>
