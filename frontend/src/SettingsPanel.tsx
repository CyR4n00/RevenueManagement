            ...
            {competitors.map((comp, index) => (
              <div key={comp.id} className="border rounded-lg p-4 bg-gray-50 flex flex-col md:flex-row items-start md:items-center space-y-3 md:space-y-0 md:space-x-4">
                 ...
                 <div className="flex-1 w-full">
                   ...
                   <input id={`comp_name_${comp.id}`} type="text" className="w-full p-2 border rounded text-sm focus:ring-2 focus:ring-blue-400 outline-none" defaultValue={comp.name} />
                 </div>
                 <div className="flex-2 w-full md:w-1/2">
                   ...
                   <input id={`comp_url_${comp.id}`} type="text" className="w-full p-2 border rounded text-sm focus:ring-2 focus:ring-blue-400 outline-none" defaultValue={comp.url} />
                 </div>
              </div>
            ))}
          </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4">
            <div className="border p-4 rounded-lg bg-green-50 border-green-200">
               ...
               <label htmlFor="line_notification" className="flex items-center space-x-2 text-sm text-gray-700 mb-2">
                 <input id="line_notification" type="checkbox" defaultChecked={true} className="rounded text-green-600 focus:ring-green-500" />
            ...
            <div className="border p-4 rounded-lg bg-gray-50">
               ...
               <label htmlFor="email_notification" className="flex items-center space-x-2 text-sm text-gray-700 mb-2">
                 <input id="email_notification" type="checkbox" defaultChecked={false} className="rounded text-blue-600 focus:ring-blue-500" />
                 <span>メールで通知を受け取る</span>
               </label>
               <label htmlFor="notification_email" className="sr-only">メールアドレス</label>
               <input id="notification_email" type="email" placeholder="example@hotel.com" className="w-full p-2 border rounded text-sm focus:ring-2 focus:ring-blue-400 outline-none mt-1" />
            </div>
          </div>
          ...
          <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
               <div>
                 <label htmlFor="notification_threshold" className="block text-xs font-bold text-gray-500 mb-1">通知を送る「価格変動」のしきい値</label>
                 <select id="notification_threshold" defaultValue="3000" className="w-full p-2 border rounded text-sm focus:ring-2 focus:ring-blue-400 outline-none bg-white">
               ...
               <div>
                 <label htmlFor="notification_timing" className="block text-xs font-bold text-gray-500 mb-1">通知のタイミング</label>
                 <select id="notification_timing" defaultValue="morning" className="w-full p-2 border rounded text-sm focus:ring-2 focus:ring-blue-400 outline-none bg-white">
