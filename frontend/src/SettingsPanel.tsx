  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden mt-6 mb-8">
      <div className="bg-gray-50 border-b p-4 flex justify-between items-center">
        ...
        <button aria-label="閉じる" onClick={onClose} className="text-gray-500 hover:text-gray-800 font-bold text-xl focus-visible:ring-2 focus-visible:outline-none">&times;</button>
      </div>
      ...
      <div className="p-6 space-y-8">
            ...
            {competitors.map((comp, index) => (
              <div key={comp.id} className="border rounded-lg p-4 bg-gray-50 flex flex-col md:flex-row items-start md:items-center space-y-3 md:space-y-0 md:space-x-4">
                 ...
                 <div className="flex-1 w-full">
                   <label htmlFor={`comp_name_${index}`} className="block text-xs font-bold text-gray-500 mb-1">施設名 (表示用)</label>
                   <input id={`comp_name_${index}`} type="text" className="w-full p-2 border rounded text-sm focus:ring-2 focus:ring-blue-400 outline-none focus-visible:ring-2" defaultValue={comp.name} />
                 </div>
                 <div className="flex-2 w-full md:w-1/2">
                   <label htmlFor={`comp_url_${index}`} className="block text-xs font-bold text-gray-500 mb-1">OTAのURL (楽天トラベル, Booking.com等)</label>
                   <input id={`comp_url_${index}`} type="text" className="w-full p-2 border rounded text-sm focus:ring-2 focus:ring-blue-400 outline-none focus-visible:ring-2" defaultValue={comp.url} />
                 </div>
              </div>
            ))}
          </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4">
            <div className="border p-4 rounded-lg bg-green-50 border-green-200">
               ...
               <label htmlFor="line_notify" className="flex items-center space-x-2 text-sm text-gray-700 mb-2">
                 <input id="line_notify" type="checkbox" defaultChecked={true} className="rounded text-green-600 focus:ring-green-500 focus-visible:ring-2" />
                 <span>LINEで通知を受け取る</span>
               </label>
               <button className="w-full bg-[#06C755] text-white font-bold py-2 rounded shadow hover:bg-green-600 transition-colors text-sm mt-2 focus-visible:ring-2 focus-visible:outline-none">
                 LINEアカウントと連携する
               </button>
            </div>
            ...
            <div className="border p-4 rounded-lg bg-gray-50">
               ...
               <label htmlFor="email_notify" className="flex items-center space-x-2 text-sm text-gray-700 mb-2">
                 <input id="email_notify" type="checkbox" defaultChecked={false} className="rounded text-blue-600 focus:ring-blue-500 focus-visible:ring-2" />
                 <span>メールで通知を受け取る</span>
               </label>
               <input id="email_address" aria-label="メールアドレス" type="email" placeholder="example@hotel.com" className="w-full p-2 border rounded text-sm focus:ring-2 focus:ring-blue-400 outline-none mt-1 focus-visible:ring-2" />
            </div>
          </div>
          ...
          <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
               <div>
                 <label htmlFor="alert_threshold" className="block text-xs font-bold text-gray-500 mb-1">通知を送る「価格変動」のしきい値</label>
                 <select id="alert_threshold" defaultValue="3000" className="w-full p-2 border rounded text-sm focus:ring-2 focus:ring-blue-400 outline-none bg-white focus-visible:ring-2">
               ...
               <div>
                 <label htmlFor="alert_timing" className="block text-xs font-bold text-gray-500 mb-1">通知のタイミング</label>
                 <select id="alert_timing" defaultValue="morning" className="w-full p-2 border rounded text-sm focus:ring-2 focus:ring-blue-400 outline-none bg-white focus-visible:ring-2">
        ...
        <div className="flex justify-end pt-4 border-t">
          <button onClick={onClose} className="bg-blue-600 text-white px-6 py-2 rounded-lg font-bold shadow hover:bg-blue-700 transition-colors focus-visible:ring-2 focus-visible:outline-none focus-visible:ring-offset-2">
            設定を保存して戻る
          </button>
        </div>
      </div>
