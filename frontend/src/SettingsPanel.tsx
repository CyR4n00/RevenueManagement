...
interface IntegrationStatus { environment: 'demo' | 'production'; apify_configured: boolean; email_delivery_configured: boolean; stripe_configured: boolean; simulation_enabled: boolean; ota_sources: OtaSource[]; }
...
export function SettingsPanel({ apiBase, accessToken, onClose }: { apiBase: string; accessToken: string; onClose: () => void }) {
  ...
  const save = async () => {
    ...
    if (competitors.some(item => !item.name?.trim() || !item.url.trim())) { setMessage('追加する競合施設の名称とOTA URLを入力してください。'); return; }
  ...
  const openPortal = async () => {
    ...
    } catch { setMessage('契約管理画面を開けませんでした。Stripe設定を確認してください。'); }
  };
  ...
  return <section role="dialog" aria-modal="true" aria-labelledby="settings-title" className="flex max-h-[92vh] w-full max-w-4xl flex-col overflow-hidden rounded-2xl border border-indigo-200 bg-white shadow-2xl">
    <div className="flex items-start justify-between gap-4 bg-gradient-to-r from-slate-950 via-indigo-950 to-violet-900 p-5 text-white"><div><p className="text-xs font-bold uppercase tracking-[0.2em] text-cyan-300">Control Center</p><h2 id="settings-title" className="mt-1 text-xl font-bold">施設設定</h2><p className="mt-1 text-sm text-indigo-100">販売ランク・競合施設・外部連携を管理します。</p></div><button onClick={onClose} aria-label="設定を閉じる" className="rounded-lg border border-white/20 px-3 py-1.5 text-xl hover:bg-white/10">×</button></div>
    <div className="overflow-y-auto p-5 md:p-6">
      ...
      <section className="mt-7 border-t pt-6"><div className="flex flex-wrap items-end justify-between gap-3"><div><h3 className="font-bold">競合施設</h3><p className="mt-1 text-xs text-slate-500">{billing.plan === 'upgrade' ? 'アップグレード' : '通常'}プラン：最大{billing.max_competitors}施設。長いURLは入力欄内でスクロールします。</p></div>{competitors.length < billing.max_competitors ? <button type="button" onClick={() => setCompetitors(items => [...items, { id: `new-${Date.now()}`, name: '', url: '', isNew: true }])} className="rounded-lg bg-indigo-50 px-3 py-2 text-sm font-bold text-indigo-700 hover:bg-indigo-100">＋ 競合施設を追加</button> : <span className="rounded-full bg-amber-50 px-3 py-1 text-xs font-semibold text-amber-700">登録上限 {competitors.length}/{billing.max_competitors}</span>}</div><div className="mt-3 space-y-3">{competitors.map(competitor => <div key={competitor.id} className="grid min-w-0 gap-3 rounded-xl border bg-slate-50 p-3 md:grid-cols-[minmax(160px,0.8fr)_minmax(0,2fr)_auto]"><label className="min-w-0 text-xs font-semibold text-slate-500">名称<input value={competitor.name || ''} placeholder="競合施設名" onChange={event => setCompetitors(items => items.map(item => item.id === competitor.id ? { ...item, name: event.target.value } : item))} className="mt-1 w-full min-w-0 rounded-lg border bg-white p-2 text-sm text-slate-800" /></label><label className="min-w-0 text-xs font-semibold text-slate-500">OTA URL<input value={competitor.url} placeholder="https://..." onChange={event => setCompetitors(items => items.map(item => item.id === competitor.id ? { ...item, url: event.target.value } : item))} className="mt-1 w-full min-w-0 rounded-lg border bg-white p-2 text-sm text-slate-800" /></label>{competitor.isNew && <button type="button" onClick={() => setCompetitors(items => items.filter(item => item.id !== competitor.id))} className="self-end rounded-lg px-3 py-2 text-sm text-red-600 hover:bg-red-50">取消</button>}</div>)}</div></section>
